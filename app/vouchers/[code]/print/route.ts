...
export async function GET(req: NextRequest, { params }: { params: { code: string } }) {
  ...
  if (error || !voucher) return new NextResponse('Not found', { status: 404 });
  ...
  const packageItems: string[] = voucher.package_notes ? voucher.package_notes.split('\n').filter((l: string) => l.trim()) : [];
...
body { font-family: Georgia, serif; background: #f0ece4; display: flex; flex-direction: column; align-items: center; padding: 32px 16px; }
